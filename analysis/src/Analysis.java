...
//        deleteOutputFileIfExists();
        ...
        deleteOutputFileIfExists("24feb/" + "failure.txt");
        deleteOutputFileIfExists("24feb/" + "comparison.csv");
        deleteOutputFileIfExists("24feb/" + "contextdiffer.csv");
        deleteOutputFileIfExists("24feb/" + "dissimilar.csv");
        deleteOutputFileIfExists("24feb/" + "24feb/dissimilar_service.csv");
        deleteOutputFileIfExists("24feb/" + "24feb/dissimilar_camera_audio.csv");
        for (String app : appList
            ...
            if (hashMapRequest.size() == 0 || hashMapUsage.size() == 0) {
                ...
                if(hashMapRequest.size() > 0 && hashMapUsage.size() > 0) {
                    ...
                    try {
                        writeResultToFile("install", "24feb/install.txt");
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                    for (String permission :
                        ...
                        if (hashMapRequest.containsKey(permission) && hashMapUsage.containsKey(permission)) {
                            try {
                                writeResultToFile("request usage found", "24feb/requsefound.txt");
                            } catch (IOException e) {
                                e.printStackTrace();
                            }
//                            HashMap<String, String> requestContext = hashMapRequest.get(permission);
...
//                                    try {
//                                        writeResultToFile(permission, "24feb/dissimilar_camera_audio.csv");
//                                    } catch (IOException e) {
//                                        e.printStackTrace();
//                                    }
//                                }
                                ...
                                try {
                                    writeResultToFile(String.valueOf(Math.abs(difference)), "24feb/dissimilar.csv");
                                } catch (IOException e) {
                                    e.printStackTrace();
                                }
                                dissimilar = true;
            ...
            if(hashMapRequest.size() > 0 || hashMapUsage.size() > 0) {
                System.out.println("\nApplication: " + app);
                try {
                    writeResultToFile("\nApplication: " + app, "24feb/" + "comparison.csv");
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
            if (hashMapRequest.size() > 0) {
                ...
                try {
                    writeResultToFile("Permission Requests", "24feb/" + "comparison.csv");
                } catch (IOException e) {
                    e.printStackTrace();
                }
                hashMapRequest.forEach((key, value) -> {
                    ...
                    try {
                        writeResultToFile(key + "\t" + value, "24feb/" + "comparison.csv");
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                });
            ...
            if (hashMapUsage.size() > 0) {
                ...
                try {
                    writeResultToFile("Permission Usage", "24feb/" + "comparison.csv");
                } catch (IOException e) {
                    e.printStackTrace();
                }
...
//            e.printStackTrace();
//        }
        deleteOutputFileIfExists("24feb/" + "requestcontext.csv");
        permissionRequestToContextCount.forEach((key, value) -> {
            try {
                findRequestContextForGroup(key, value);
                writeResultToFile(key + "\t" + value, "24feb/" + "requestcontext.csv");
            } catch (IOException e) {
                e.printStackTrace();
            }
        });
...
//            e.printStackTrace();
//        }
        deleteOutputFileIfExists("24feb/" + "usagecontext.csv");

        permissionUsageToContextCount.forEach((key, value) -> {
            try {
                findUsageContextForGroup(key, value);
                writeResultToFile(key + "\t" + value, "24feb/" + "usagecontext.csv");
            } catch (IOException e) {
                e.printStackTrace();
            }
        });

        deleteOutputFileIfExists("24feb/" + "foreground_background.csv");
        permissionUseInBackGroundAndForegound.forEach((key, value) -> {
            try {
                writeResultToFile(key + "\t" + value, "24feb/" + "foreground_background.csv");
            } catch (IOException e) {
                e.printStackTrace();
            }
        });

        deleteOutputFileIfExists("24feb/" + "foreground.csv");
        permissionUseForegound.forEach((key, value) -> {
            try {
                writeResultToFile(key + "\t" + value, "24feb/" + "foreground.csv");
            } catch (IOException e) {
                e.printStackTrace();
            }
        });

        deleteOutputFileIfExists("24feb/" + "background.csv");
        permissionUseInBackGround.forEach((key, value) -> {
            try {
                writeResultToFile(key + "\t" + value, "24feb/" + "background.csv");
            } catch (IOException e) {
                e.printStackTrace();
            }
        });
        ...
        deleteOutputFileIfExists("24feb/" + "request_group.csv");
        permissionGroupRequestContextCount.forEach((key, value) -> {
            try {
                writeResultToFile(key + "\t" + value, "24feb/" + "request_group.csv");
            } catch (IOException e) {
                e.printStackTrace();
            }
        });

        deleteOutputFileIfExists("24feb/" + "usage_group.csv");
        permissionGroupUsageContextCount.forEach((key, value) -> {
            try {
                writeResultToFile(key + "\t" + value, "24feb/" + "usage_group.csv");
            } catch (IOException e) {
                e.printStackTrace();
            }
        });
        ...
        deleteOutputFileIfExists("24feb/" + "app_stats.csv");
        categoryStats.forEach((key, value) -> {
            try {
                writeResultToFile(key + "\t" + value, "24feb/" + "app_stats.csv");
            } catch (IOException e) {
                e.printStackTrace();
            }
        });
        deleteOutputFileIfExists("24feb/" + "dissimilar_app_stats.csv");
        dissimilarCategoryStats.forEach((key, value) -> {
            try {
                writeResultToFile(key + "\t" + value, "24feb/" + "dissimilar_app_stats.csv");
            } catch (IOException e) {
                e.printStackTrace();
            }
        });
        deleteOutputFileIfExists( "24feb/" + "dissimilarContextGroup.csv");
        dissimilarContextGroup.forEach((key, value) -> {
            try {
                writeResultToFile(key + "\t" + value, "24feb/" + "dissimilarContextGroup.csv");
            } catch (IOException e) {
                e.printStackTrace();
            }
        });
        deleteOutputFileIfExists( "24feb/" + "dissimilarContextPermission.csv");
        dissimilarContextPermission.forEach((key, value) -> {
            try {
                writeResultToFile(key + "\t" + value, "24feb/" + "dissimilarContextPermission.csv");
            } catch (IOException e) {
                e.printStackTrace();
            }
        });
...
//                    }
    ...
    private static boolean isFragmentLifeCycle(String s) {
        for (String method :
                fragmentLifecycleMethods) {
    ...
    private static boolean isBroadcastLifeCycle(String s) {
        for (String method :
                broadCastLifeCycleMethods) {
            if (s.contains(method)) {
                return true;
            }
        }
    ...
    private static boolean isServiceLifeCycle(String s) {
        for (String method :
                serviceLifecycleMethods) {
            if (s.contains(method)) {
                return true;
            }
        }
    ...
    private static boolean isActivityLifeCycle(String s) {
        for (String method :
    ...
    private static void readRequests() {
        try {
            File file = new File("24feb/" + "output_request.csv");
            FileReader fileReader = new FileReader(file);
            BufferedReader bufferedReader = new BufferedReader(fileReader);
            String line;
            String[] tokens;
    ...
    private static void readUsage() {
        try {
            File file = new File("24feb/" + "output_usage.csv");
            FileReader fileReader = new FileReader(file);
            BufferedReader bufferedReader = new BufferedReader(fileReader);
            String line;
            String[] tokens;
    ...
    private static void openPermissionFile() {
        try {
            File file = new File("24feb/" + "uniq_apps1");
            FileReader fileReader = new FileReader(file);
            BufferedReader bufferedReader = new BufferedReader(fileReader);
            String line;
            String[] token;
